...
### TASK-09 — Integração Preparar Máquina com Deploy (Implementado)
- **Status:** Implementação e validação humana principal concluídas; fechamento documental realizado, aguardando validação formal externa.
- **Entregáveis:** opção Reiniciar depois; estado `rebootAfterDeploy`; indicador global persistente; direcionamento ao executor Deploy existente; baseline configurável por `defaultForPreparation`; pré-seleção one-shot sem autoexecução; aviso obrigatório pós-fila com Reiniciar agora ou Adiar.
- **Correções finais:** `runDeployItemTracked` cria e rastreia o processo antes dos listeners, eliminando `ReferenceError: proc is not defined`; Script `.bat/.cmd` e Executável `.exe/.msi` usam `cmd.exe`, preservam exit real e aguardam `close`; Shell é explicitamente fire-and-forget, retorna estado `aberto; término não rastreável` e nunca conta como conclusão. Script possui `showConsole` opcional: falso mantém console oculto com stdout/stderr CP850 no terminal interno; verdadeiro rastreia um wrapper `cmd.exe` que usa `start /wait` para abrir e aguardar o CMD interativo, propagando término e permitindo cancelar toda a árvore pelo PID do wrapper. Resultado final distingue sucesso, sucesso parcial, falha total, cancelamento e itens apenas abertos; orientação para Catálogo aparece somente para erros identificados como configuração/path; modal preserva simultaneamente resultado da fila e reboot pendente; botões mantêm contraste normal, hover, focus-visible e disabled.
- **Teclado/foco:** o handler global ignora `input`, `textarea`, `select` e `contentEditable`; o terminal somente ajusta scroll e não chama `focus`; controles editáveis são explicitamente `-webkit-app-region: no-drag` e permitem seleção de texto na janela frameless; formulários de software e hostname solicitam foco inicial nativo; o hostname usa submit local para Enter executar somente a validação habilitada, sem avançar para rename/reboot.
- **Paths:** caminhos configurados têm representação canônica sem aspas externas; um par válido colado pelo usuário é removido centralmente na leitura, salvamento, Testar e execução, enquanto aspas incompletas/comando misturado são rejeitados. Para Script/Executável, o executor usa `cmd.exe /d /s /c` com a command line externa exigida pelo CMD e `windowsVerbatimArguments: true`, evitando que o Node transporte `\"` como texto literal.
- **Collapse:** a janela reduz temporariamente seu `minimumSize` para a altura do header antes de recolher e restaura o mínimo suportado ao expandir, eliminando o espaço vazio causado pelo `minHeight` nativo de 380px.
- **Segurança:** IPC de adiamento aceita somente payload vazio; rename/reboot da TASK-08 preservados; Deploy continua usando a fila sequencial existente; estado só é limpo quando hostname ativo corresponde ao esperado.
