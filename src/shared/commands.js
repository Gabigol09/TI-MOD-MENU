...
export const CATEGORIES = [
  ...
  },
  {
    ...
    cmds: [
      { name: 'Office 365 (Notebook)',   desc: 'OfficeSetup.exe',                               type: 'path',   cmd: '\\\\servidor\\soft\\Utilitarios\\EasyDeploy\\OfficeSetup.exe', buildCmd: cfg => cfg?.paths?.office365, tip: 'Notebooks - Office 365 atualizado (caminho definido no config.json)' },
      { name: 'Office 2016 (Desktop)',   desc: 'Office2016_PRO.xml x64',                        type: 'open',   cmd: '"\\\\servidor\\soft\\Licenciados\\Microsoft Office\\2016\\PRO\\PT-BR\\x64\\setup.exe" /config "\\\\servidor\\soft\\Utilitarios\\EasyDeploy\\SoftwaresBasicos\\Office2016_PRO.xml"', buildCmd: cfg => cfg?.paths?.office2016 && `"${cfg.paths.office2016}" /config "${cfg.paths.office2016Config}"`, tip: 'Desktops / mini PCs - Office 2016 Pro' },
      ...
      { name: 'Google Chrome',           desc: 'ChromeSetup.exe /silent',                       type: 'open',   cmd: '"\\\\servidor\\soft\\Utilitarios\\EasyDeploy\\SoftwaresBasicos\\ChromeSetup.exe" /silent /install', buildCmd: cfg => cfg?.paths?.chrome && `"${cfg.paths.chrome}" /silent /install`, tip: 'Navegador padrao' },
      { name: 'Microsoft Teams',         desc: 'MSTeamsSetup.exe',                              type: 'path',   cmd: '\\\\servidor\\soft\\Utilitarios\\EasyDeploy\\MSTeamsSetup.exe', buildCmd: cfg => cfg?.paths?.teams, tip: 'Instalador do Teams' },
